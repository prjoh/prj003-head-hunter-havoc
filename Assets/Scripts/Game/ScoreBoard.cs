...
public class ScoreBoard : MonoBehaviour, ISaveable
{
    ...
    private void RemoveHighlight()
    {
        foreach (var scoreObj in _scoreEntries)
        {
            ...
            scoreEntry.m_Playername.readOnly = true;
            scoreEntry.m_Playername.onFocusSelectAll = false;
        }
    }
    ...
    public void UpdateScoreboard(int score, string timeText, string name = "")
    {
        ...
        scoreEntryScript.m_Playername.ActivateInputField();

        if (name.Length > 0)
            scoreEntryScript.m_Playername.text = name;
        else
            scoreEntryScript.m_Playername.text = "head hunter";
        scoreEntryScript.score.text = score.ToString();
        scoreEntryScript.time.text = timeText;

        StartCoroutine(SetCaret(scoreEntryScript.m_Playername));
    ...
    public void PopulateSaveData(SaveData a_SaveData)
    {
        ...
        foreach (var scoreObj in _scoreEntries)
        {
            ...
            var scoreData = new SaveData.ScoreData
            {
                ...
                name = scoreEntry.m_Playername.text,
    ...
    public void LoadFromSaveData(SaveData a_SaveData)
    {
        foreach (var scoreData in a_SaveData.scores)
        {
            ...
            scoreEntry.m_Playername.text = scoreData.name;
