Shader "Off Axis Studios/Low Poly Style Flat (Color Only)"
{
	Properties
	{
		...
		SubShader
		{
			...
			Pass
			{
				...
				half4 frag(g2f IN) : COLOR
				{
					...
					return float4((IN.vertexLighting + (ambientLight * _LightIntensityMultiplier) + diffuseReflection + specularReflection) * _Color, 1);
				}
				...
				Pass
				{
					...
					float4 frag(g2f IN) : COLOR
					{
						...
						return float4((diffuseReflection + specularReflection), 1);
					}
					ENDCG
				}
		}
